...
function devIdentityService(roles) {
    const config = {
...
async function deviceLogin(identity, providerId, role) {
...
// validate
// ---------------------------------------------------------------------------
export function validateCommand(packageDir) {
    try {
        const pkg = loadWorkforcePackage(packageDir);
        console.log(`✔ ${pkg.manifest.name} v${pkg.manifest.version} (${pkg.manifest.id}) is valid`);
        console.log(`  agents: ${pkg.agents.size}, skills: ${pkg.skills.size}, personas: ${pkg.personas.size}, workflows: ${pkg.workflows.size}`);
        return 0;
    }
    catch (error) {
...
// inspect
// ---------------------------------------------------------------------------
export function inspectCommand(packageDir) {
    try {
        const pkg = loadWorkforcePackage(packageDir);
        ...
        console.log('\nPersonas:');
        for (const persona of pkg.personas.values())
            console.log(`  ${persona.id} — ${persona.tone ?? ''}`);
        console.log('\nWorkflows:');
...
// run
// ---------------------------------------------------------------------------
export async function runCommand(packageDir, workflowId, options = {}) {
    const pkg = loadWorkforcePackage(packageDir);
    ...
    const answers = options.answersPath
        ? JSON.parse(readFileSync(options.answersPath, 'utf8'))
        : [];
    let answerIndex = 0;
    ...
    if (options.watch)
        console.log(`Run ${run.id} started (${run.status})`);
    while (run.status === 'waitingForHuman' && run.pending) {
        ...
        if (options.watch)
            console.log(`  ↳ resumed → ${run.status}`);
    }
...
// runs list / runs show
// ---------------------------------------------------------------------------
export function runsListCommand(options) {
    ...
    const statusIcon = (s) => s === 'completed' ? '✔' : s === 'failed' ? '✘' : s === 'waitingForHuman' ? '⏸' : '⟳';
    for (const run of runs) {
        console.log(`${statusIcon(run.status)} ${run.id}  ${run.workflowId}  [${run.packageId}]  ${run.status}${run.pending ? `  ← waiting for ${run.pending.role}` : ''}`);
    }
    return 0;
}
export async function runsShowCommand(runId, options) {
    const kernel = new FlowForgeKernel({ dataDir: options.dataDir ?? defaultDataDir() });
...
export function auditShowCommand(options) {
    ...
    for (const record of trail.records) {
        ...
        if (record.workflowRunId)
            parts.push(`run=${record.workflowRunId.slice(0, 8)}`);
        if (record.nodeId)
            parts.push(`@${record.nodeId}`);
        console.log(parts.join('  '));
    }
    console.log(`\nchain: ${trail.chainIntact ? 'intact ✔' : 'BROKEN ✘'} (${trail.records.length} records)`);
    return trail.chainIntact ? 0 : 1;
}
export function auditVerifyCommand(options) {
    const kernel = new FlowForgeKernel({ dataDir: options.dataDir ?? defaultDataDir() });
    const trail = kernel.getAuditTrail();
...
// ---------------------------------------------------------------------------
export async function memoryListCommand(namespace, 
...
export async function memoryDeleteCommand(namespace, itemId, 
...
function positionals(args, ...flagNames) {
    const result = [];
...
if (isDirectRun) {
    ...
    switch (command) {
        ...
        case 'runs': {
            const sub = subOrArg;
            if (sub === 'list') {
                process.exit(runsListCommand({ dataDir: flag(rest, '--data-dir'), packageId: flag(rest, '--package') }));
            }
            else if (sub === 'show') {
        ...
        case 'audit': {
            ...
            if (sub === 'show') {
                ...
                process.exit(auditExportCommand({
                    runId: flag(rest, '--run'),
        ...
        case 'memory': {
            ...
            if (sub === 'list') {
                ...
                memoryListCommand(pos[0], { dataDir: flag(rest, '--data-dir') }).then((code) => process.exit(code));
            }
            else if (sub === 'delete') {
                if (!pos[0] || !pos[1]) {
                    usage();
                ...
                memoryDeleteCommand(pos[0], pos[1], { dataDir: flag(rest, '--data-dir') }).then((code) => process.exit(code));
            }
            else {
                usage();
                process.exit(1);
