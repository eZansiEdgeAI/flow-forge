 ...
 */
export function parseSkillFile(path, label) {
...
function assertValid(schema, doc, label) {
    ...
    if (!result.valid)
        throw new PackageValidationError(`Invalid ${label}`, result.errors);
}
/**
 ...
 */
export function loadWorkforcePackage(packageDir) {
    const rootDir = resolve(packageDir);
    const manifest = readJson(join(rootDir, 'workforce.json'));
    ...
    for (const relPath of manifest.personas ?? []) {
        const persona = readJson(join(rootDir, relPath));
        assertValid('persona', persona, relPath);
        personas.set(persona.id, persona);
    }
    const workflows = new Map();
    for (const relPath of manifest.workflows) {
        const workflow = readJson(join(rootDir, relPath));
    ...
    for (const agent of agents.values()) {
        for (const skillId of agent.skills ?? []) {
            if (!skills.has(skillId))
                crossErrors.push(`agent '${agent.id}' references unknown skill '${skillId}'`);
        }
    ...
    for (const workflow of workflows.values()) {
        ...
        for (const node of workflow.nodes) {
            ...
            if (node.type === 'humanApproval')
                targets.push(node.onApprove, node.onReject);
            if (node.type === 'branch')
                targets.push(...node.conditions.map((c) => c.next));
